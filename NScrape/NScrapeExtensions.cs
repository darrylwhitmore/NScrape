...
namespace NScrape {
	...
	public static class NScrapeExtensions {
		...
		public static Encoding GetEncoding( this HttpWebResponse httpWebResponse ) {
			...
			var characterSet = !string.IsNullOrEmpty( httpWebResponse.CharacterSet ) ? httpWebResponse.CharacterSet : "iso-8859-1";
		...
		/// </returns>
		public static string GetResponseText( this HttpWebResponse httpWebResponse, Encoding encoding = null ) {
			if ( encoding == null ) {
				encoding = httpWebResponse.GetEncoding();
			}
			...
			if ( s != null ) {
				...
				if ( httpWebResponse.Headers[HttpResponseHeader.ContentEncoding] == "gzip" ) {
